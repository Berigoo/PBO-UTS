...
public abstract class Pengguna {
    ...
    public int getDaftarBukuByGenre(List<Buku> db, String[] genre){     //return jumlah buku yang memenuhi
        String left = "";
        String right = "";
        for (int i=0; i<db.size(); i++){                        // Sortir berdasar genre (apakah buku tersebut terdapat genre yang diminta)
            boolean temp = true;                                // jika iya pindah ke left jika tidak pindah ke right (index)
        ...
        for (int i=0; i<db.size(); i++){                        //Printing
            System.out.println((i+1) + ".");
